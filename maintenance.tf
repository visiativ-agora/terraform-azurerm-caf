...
module "maintenance_assignment_virtual_machine" {
  ...
  virtual_machine_id = can(each.value.virtual_machine_id) ? each.value.virtual_machine_id : local.combined_objects_virtual_machines[try(each.value.resource_group_key, local.resource_groups[each.value.resource_group_key].name)][each.key].id
