...
# Name of the Windows computer name - 15 chars max
data "azurecaf_name" "windows_computer_name" {
  for_each = local.os_type == "windows" ? var.settings.virtual_machine_settings : {}
...
data "azurecaf_name" "os_disk_windows" {
  for_each = local.os_type == "windows" ? var.settings.virtual_machine_settings : {}
...
resource "azurerm_windows_virtual_machine" "vm" {
  ...
  #bypass_platform_safety_checks_on_user_schedule_enabled = (each.value.patch_mode == "AutomaticByPlatform" ? true : try(each.value.bypass_platform_safety_checks_on_user_schedule_enabled, null))
  bypass_platform_safety_checks_on_user_schedule_enabled = (can(each.value.patch_mode) && lookup(each.value, "patch_mode") == "AutomaticByPlatform" ? true : try(each.value.bypass_platform_safety_checks_on_user_schedule_enabled, null))
