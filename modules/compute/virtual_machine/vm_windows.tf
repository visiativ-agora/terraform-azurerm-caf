...
resource "azurerm_windows_virtual_machine" "vm" {
  ...
  #bypass_platform_safety_checks_on_user_schedule_enabled = (can(each.value.patch_mode) && each.value.patch_mode == "AutomaticByPlatform" ? true : can(each.value.bypass_platform_safety_checks_on_user_schedule_enabled) ? each.value.bypass_platform_safety_checks_on_user_schedule_enabled : null)
