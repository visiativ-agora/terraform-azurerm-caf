resource "azurerm_site_recovery_replicated_vm" "replication" {
  ...
  managed_disk {
    ...
    target_disk_type         = lookup(var.settings.replication.target, "os_disk_storage_type", var.virtual_machine_os_disk.storage_account_type) # When I retrieve the storage account type, the plan detects a change and rebuilds the replication item. If anyone has an idea how to solve this problem
    target_replica_disk_type = lookup(var.settings.replication.target, "os_disk_replica_storage_type", lookup(var.settings.replication.target, "os_disk_storage_type", var.virtual_machine_os_disk.storage_account_type))
