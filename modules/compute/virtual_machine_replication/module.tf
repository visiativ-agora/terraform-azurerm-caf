resource "azurerm_site_recovery_replicated_vm" "replication" {
  ...
  managed_disk {
    ...
    target_disk_encryption_set_id = contains(keys(var.disk_encryption_sets[try(var.settings.os_disk.lz_key, var.client_config.landingzone_key)]), var.settings.os_disk.disk_encryption_set_key) ? var.disk_encryption_sets[try(var.settings.os_disk.lz_key, var.client_config.landingzone_key)][var.settings.os_disk.disk_encryption_set_key].id : null
