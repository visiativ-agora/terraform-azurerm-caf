...
resource "azapi_update_resource" "approve_private_endpoint" {
  ...
  type        = "Microsoft.Sql/servers/privateEndpointConnections@2024-05-01-preview"
  resource_id = "${var.mssql_servers[try(var.settings.lz_key, var.client_config.landingzone_key)][var.settings.mssql_server_key].id}/privateEndpointConnections/${try(local.private_endpoint_connection_name, "default_value")}"
  # name = local.private_endpoint_connection_name
  body = jsonencode({
    properties = {
      privateLinkServiceConnectionState = {
