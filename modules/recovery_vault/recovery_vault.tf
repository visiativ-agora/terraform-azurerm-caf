...
resource "azurerm_recovery_services_vault" "asr" {
  ...
  dynamic "encryption" {
    ...
    content {
      ...
      user_assigned_identity_id         = var.managed_identities[try(var.settings.encryption.user_assigned_identity.lz_key, var.client_config.landingzone_key)][var.settings.encryption.user_assigned_identity.key].principal_id
    }
  }
}
